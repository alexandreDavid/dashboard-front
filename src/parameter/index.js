...
export default {
  async getAllParameters () {
    ...
    parameters = structure.parameters.filter(p => p.isActive).map(p => {
      p.layerUrl = '/geoserver/wms'
      p.layerParameters = {
        layers: `${p.workspaceName}:${p.paramName}`,
        format: 'image/png',
        transparent: true
      }
      p.legendUrl = `/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&LAYER=${p.workspaceName}:${p.paramName}&legend_options=fontName:Roboto%20Light;fontAntiAliasing:true;fontColor:0x333132;fontSize:10;bgColor:0xFFFFFF;dpi:180;&format_options=layout:legend;`
      return p
    })
    return parameters
  },
